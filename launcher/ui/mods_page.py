...
class ModsPage(QWidget):
    ...
    def _build(self):
        ...
        self.btn_gh = QPushButton(i18n.tr("btn_refresh"))
        self.btn_gh.setProperty("ghost", True)
        self.btn_gh.clicked.connect(self.load_github)
        ...
        self.btn_mr = QPushButton(i18n.tr("search"))
        self.btn_mr.setProperty("ghost", True)
        self.btn_mr.clicked.connect(self.search_modrinth)
        srow.addWidget(self.mr_search, 1)
        srow.addWidget(self.btn_mr)
        ml.addLayout(srow)
        ...
        self.btn_mp = QPushButton(i18n.tr("search"))
        self.btn_mp.setProperty("ghost", True)
        self.btn_mp.clicked.connect(self.search_modpacks)
        self.btn_imp = QPushButton(i18n.tr("modpack_import"))
        self.btn_imp.setProperty("ghost", True)
        self.btn_imp.clicked.connect(self.import_modpack)
        prow.addWidget(self.mp_search, 1)
        prow.addWidget(self.btn_mp)
        prow.addWidget(self.btn_imp)
        mpl.addLayout(prow)
        ...
        self.btn_sh = QPushButton(i18n.tr("search"))
        self.btn_sh.setProperty("ghost", True)
        self.btn_sh.clicked.connect(self.search_shaders)
        srow2.addWidget(self.sh_search, 1)
        srow2.addWidget(self.btn_sh)
        shl.addLayout(srow2)
    ...
    def retranslate(self):
        ...
        self.btn_gh.setText(tr("btn_refresh"))
        self.btn_mr.setText(tr("search"))
        self.btn_mp.setText(tr("search"))
        self.btn_imp.setText(tr("modpack_import"))
        self.btn_sh.setText(tr("search"))
    ...
    def search_modpacks(self):
        ...
        def ok(hits):
            ...
            for h in hits:
                ...
                nm.setStyleSheet("font-size:13px; color:#EAF0FF;")
                info = QLabel(i18n.tr("modpack_tag") + f" · {h.get('downloads', 0)} DL")
                info.setProperty("subtitle", True)
    ...
    def search_shaders(self):
        ...
        def ok(hits):
            ...
            for h in hits:
                ...
                nm = QLabel(h.get("title", "?"))
                nm.setStyleSheet("font-size:13px; color:#EAF0FF;")
                info = QLabel(i18n.tr("shader_tag") + f" · {h.get('downloads', 0)} DL")
                info.setProperty("subtitle", True)
...
def utils_http_get(url: str, timeout: int = 120) -> bytes:
    """下载整合包文件到内存（供 mrpack 解析），带 UA 与重试。"""
    from .. import utils as _u
    return _u.http_get_bytes(url, timeout=timeout)
